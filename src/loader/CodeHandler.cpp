...
{
// From Gecko OS (GPLv2)
constexpr std::array<u8, 4288> s_code_handler = {{
...
{
  // Unused memory area.
  u8* dest = reinterpret_cast<u8*>(0x80001800);

  // TODO[config]: Allow the user to choose which variant to use (slot A or B).
  // The currently used variant has debugger functionality and only works with slot B.
  std::copy(s_code_handler.cbegin(), s_code_handler.cend(), dest);
  Common::SyncAfterWrite(dest, s_code_handler.size());
